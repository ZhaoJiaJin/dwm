...
keyrelease(XEvent *e)
{
	if ((oldshowbar != selmon->showbar) && (e->xkey.keycode == XKeysymToKeycode(dpy, XK_Super_L))) {
		selmon->showbar = 0;
		updateholdbarpos(selmon);
		XMoveResizeWindow(dpy, selmon->barwin, selmon->wx, selmon->by, selmon->ww, bh);
		//arrange(selmon);
...
drawbar(Monitor *m)
{
  int x, w, tw = 0, n = 0, scm;
	...
	if (m == selmon) { /* status is only drawn on selected monitor */
		...
		drw_text(drw, m->ww - sw - stw, 0, tw, bh, lrpad / 2 - 2, stext, 0);
	}
  ...
  if (display != NULL) {
    ...
    if (resm != NULL) {
      ...
      if (xrdb != NULL) {
        ...
        XRDB_LOAD_COLOR("dwm.hidbordercolor", urg_border);
        XRDB_LOAD_COLOR("dwm.hidbgcolor", urg_bg);
        XRDB_LOAD_COLOR("dwm.hidfgcolor", urg_fg);
      }
    }
  }
